...
Forest provides an integrated version, LeafImmer, that uses the immutable library Immer
to establish immutable values for all complex types; meaning,
...
The fact that Forest instances are self-contained makes them much more amenable to basic test introspection.
...
I don't have to lose the ability to stop a change from occurring or be a slave to the tempo of my state system.
These signals include change requests as well as specific user actions, next signals and all meaningful
triggers inside the forest system.
